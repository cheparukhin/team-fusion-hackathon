...
def save(fig):
    ...
    for ext in ('svg','png','pdf'):
        path = FIG / f"{item['slug']}.{ext}"
        fig.savefig(path,dpi=150,bbox_inches='tight',facecolor='white')
        item[ext] = str(path.relative_to(ROOT))
    if fig._suptitle is not None: fig._suptitle.set_visible(False)
    slide_path=FIG/f"{item['slug']}.slide.png";fig.savefig(slide_path,dpi=150,bbox_inches='tight',facecolor='white');item['slide_png']=str(slide_path.relative_to(ROOT))
    plt.close(fig)
...
def render():
    ...
    sections=[]
    for item in FIGURES:
        sections.append(f'<section id="{item["slug"]}"><h2>{html.escape(item["title"])}</h2><img src="../figures/{item["slug"]}.svg" alt="{html.escape(item["title"])}"><p><b>Method.</b> {html.escape(item["method"])}</p><p><b>Interpretation.</b> {html.escape(item["interpretation"])}</p><small>Source: {html.escape(item["source"])}</small><p><a href="../figures/{item["slug"]}.png">PNG</a> · <a href="../figures/{item["slug"]}.svg">SVG</a> · <a href="../figures/{item["slug"]}.pdf">PDF</a></p></section>')
    ...
    for item in FIGURES: md += [f"## {item['title']}",'',f"![{item['title']}](../figures/{item['slug']}.png)",'',f"Source: {item['source']}",'',f"Method: {item['method']}",'',f"Interpretation: {item['interpretation']}",'']
...
if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('--render-pptx',action='store_true');parser.add_argument('--check-browser',action='store_true');args=parser.parse_args()
