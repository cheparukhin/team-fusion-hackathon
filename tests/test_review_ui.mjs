...
function environment(storage=new Map(), deny=false){
 const elements=new Map(),downloads=[],focusLog=[];
 function element(id){if(!elements.has(id))elements.set(id,{id,value:'',hidden:false,innerHTML:'',textContent:'',validation:'',listeners:{},addEventListener(n,f){this.listeners[n]=f;},setCustomValidity(v){this.validation=v;},scrollIntoView(){},focus(){focusLog.push(this.id);},querySelector(selector){return element(`${this.id}:${selector}`);},reportValidity(){return ['reviewer','note','verdict'].every(n=>element(n).value&&!element(n).validation);}});return elements.get(id);}
 ...
 return {ctx,element,downloads,storage,focusLog,run:s=>vm.runInContext(s,ctx),submit(){element('review-form').listeners.submit({preventDefault(){}});}};
}
const e=environment();
...
assert.equal(e.focusLog.at(-1),'cases:[data-case="psap-lgals3"]');
e.element('step-nav').listeners.click({target:{closest(){return {dataset:{step:'1'}};}}});
assert.equal(e.focusLog.at(-1),'step-nav:[data-step="1"]');
assert.match(e.element('step-detail').innerHTML,/Closest endpoint discrepancy/);
e.element('claim-list').listeners.click({preventDefault(){},target:{closest(){return {dataset:{evidence:'2'}};}}});
assert.equal(e.focusLog.at(-1),'step-detail');
assert.match(e.element('step-detail').innerHTML,/Non-detection does not establish absence/);
...
console.log('Review application logic passed: three cases, empty-field rejection, persistence, case binding, export, clear, unavailable storage, stale record rejection, text escaping, keyboard focus handoff. Visual/browser verification not performed.');
