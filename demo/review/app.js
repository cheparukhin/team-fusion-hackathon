...
const subtitles={'psap-lgals3':'High score · junction conflict','gsdmd-tmem106a':'RNA support · uncertain fold','cd274-lacc1':'RNA support · missing structure'};
...
function renderEvidence(){
 ...
 if(current.slug==='psap-lgals3'){
  ...
  html=`<div class="eyebrow">A JUNCTION TO RESOLVE</div><div class="evidence-number">${fmt(p.closest_max_offset_nt)}<span>nt</span></div><p class="evidence-caption">between the nearest recorded read endpoint and the designed probe on the second parent.</p><div class="junction-line"><div>Nearest read · chr14<strong>${fmt(nearest.published_breakpoint2)}</strong></div><div>Designed probe · chr14<strong>${fmt(p.probe_breakpoint2)}</strong></div></div>${facts([[c.long_read_support,'Published read IDs · pair-level'],[fmt(c.score_rna,3),'RNA score · not a probability'],['Unknown','Per-probe testing / QC']])}<div class="caution">The score ranks the pair. It does not resolve which junction the probe tests. Check the original read alignment and coordinate convention before interpreting the discrepancy.</div>`;
 }else if(structure?.status==='available'){
  const confidence=structure.predictions[0].confidence.mean_plddt;
  html=`<div class="eyebrow">PUBLISHED RNA / PREDICTED STRUCTURE</div><div class="evidence-number">${fmt(confidence,1)}<span>pLDDT</span></div><p class="evidence-caption">Mean confidence of the cached Boltz2 chimera model. Its fold remains uncertain.</p><div class="confidence-track" aria-label="Mean model confidence ${fmt(confidence,1)} out of 100"><span class="confidence-marker" style="left:${Math.max(0,Math.min(100,confidence))}%"></span></div><div class="confidence-labels"><span>0 · lower confidence</span><span>100 · higher confidence</span></div>${facts([['Reported','Published NanoString support'],[gpu.support,'Matching reads · bounded pilot'],[fmt(comparison.comparisons[0].closest_max_offset_nt)+' nt','Read / probe endpoint offset']])}<div class="caution">Published RNA evidence is separate from the predicted structure. Neither this low-confidence fold nor pilot non-detection settles protein presence or function.</div>`;
 }else{
