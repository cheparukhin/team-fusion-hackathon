...
def preflight():
    ...
    if any(w['name']==NAME for w in inventory):raise RuntimeError('Owned name already exists')
    request=urllib.request.Request(transport.URL,data=json.dumps({'options':{'includeCpu':True,'skipAccessFilter':False}}).encode(),headers={'Content-Type':'application/json','Connect-Protocol-Version':'1'})
    quotes=json.load(urllib.request.urlopen(request,timeout=60))
    save('launch-live-quotes.json',{'utc':transport.now(),'response':quotes})
    save('launch-inventory.json',{'utc':transport.now(),'workspaces':inventory})
    prices=[]
    bounds={'n2d-standard-16':16384,'n2d-standard-128':16384,'n2d-highmem-16':16384,'cpu-e2.4vcpu-16gb':2560,'a100-80gb.1x':10368,'a100-80gb-sxm-ib.8x':10368}
    for w in inventory+[{'name':NAME,'instance_type':TYPE,'status':'PROPOSED'}]:
        rows=[q for q in quotes['items'] if q['type']==w['instance_type']]
        if len(rows)!=1:raise RuntimeError('Unpriced SKU')
        q=rows[0];money=[q['basePrice'],*q.get('locationPrices',{}).values()]
        storage=[v.get('pricePerGbHr') for v in q['supportedStorage']]
        if not storage or any(not m or m['currency']!='USD' for m in money+storage):raise RuntimeError('Unknown currency/storage rate')
        if w['name']==NAME and (q['provider']!='crusoe' or not q.get('isAvailable') or not q.get('stoppable')):raise RuntimeError('Worker unavailable')
        disk=DISK_GIB_BOUND if w['name']==NAME else bounds[w['instance_type']]
        compute=0 if w['status'] in ('STOPPED','DELETED') else max(float(m['amount']) for m in money)
        rate=compute+math.ceil(disk*1024**3/10**9)*max(float(m['amount']) for m in storage)
        prices.append({'name':w['name'],'usd_per_hour_upper_bound':rate,'currency':'USD','storage_gib_bound':disk,'type':w['instance_type']})
    spec=importlib.util.spec_from_file_location('budget',ROOT/'scripts/check_budget.py');m=importlib.util.module_from_spec(spec);spec.loader.exec_module(m)
    result=m.check(json.loads((ROOT/'infra/budget-policy.json').read_text()),[p['usd_per_hour_upper_bound'] for p in prices[:-1]],prices[-1]['usd_per_hour_upper_bound'])
    if prices[-1]['usd_per_hour_upper_bound']*3>10:raise RuntimeError('New worker three-hour cap exceeds $10')
    if sum(p['usd_per_hour_upper_bound'] for p in prices)*3>450:raise RuntimeError('Insufficient project credit reserve')
    save('launch-budget.json',{'utc':transport.now(),'resources':prices,'check':result,'new_worker_max_hours':3,'new_worker_max_usd':prices[-1]['usd_per_hour_upper_bound']*3,'note':'Conservative quote bound, not reconciled provider billing. Prior K562 worker upper quote $2.524; combined K562 allowance $20. Other active jobs untouched.'})
    return result
...
def validate_inputs(inputs):
    ...
    if not freeze['frozen']:raise RuntimeError('Selection is not frozen')
...
def main(inputs):
    ...
    try:
        ...
        with tarfile.open(bundle,'w:gz') as archive:
            ...
            preparation=ROOT/'runs/focused-pilot-20260919/boltz-preparation'
        ...
        save('input-bundle.json',{'utc':transport.now(),'sha256':hashlib.sha256(bundle.read_bytes()).hexdigest(),
            ...
            'source_hashes':{name:hashlib.sha256((ROOT/'scripts'/name).read_bytes()).hexdigest()
                             for name in ('k562/run_all_folds.py','boltz_worker.py','setup_boltz_worker.sh')}})
