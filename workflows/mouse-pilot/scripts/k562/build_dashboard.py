...
import csv,gzip,hashlib,json,re,collections,os
ROOT=Path(__file__).resolve().parents[2];RUN=ROOT/'runs/k562-pilot/20260919-overnight';OUT=RUN/'outputs';DEST=ROOT/'reports/k562-dashboard'
VIS=Path(os.environ.get('CHRNA_VIS_DIR', str(DEST)))/'k562-rna-evidence.html'
...
def main():
 ...
 gtf=DEST/'gencode.v43.annotation.gtf.gz';ref=load(OUT/'provenance/gencode.v43.annotation.gtf.gz.json');assert sha(gtf)==ref['sha256']
 ...
 for jid in sorted(ids,key=lambda j:next(r['display_rank'] for r in rankings[0] if r['junction_id']==j)):
  sample=next(d for d in decisions[0] if d['junction_id']==jid)
  rna=[]
  ...
  supports=[next(r for r in rs if r['junction_id']==jid) for rs in rankings]
  ...
  candidates.append({'id':jid,'pair':sample['gene_name_5p']+' → '+sample['gene_name_3p'],'coordinates':{k:sample[k] for k in ['reference_build','gene_5p','gene_3p','chromosome_5p','chromosome_3p','boundary_5p','boundary_3p','strand_5p','strand_3p']},'support':[r['distinct_qualifying_reads'] for r in supports],'rank':[r['display_rank'] for r in supports],'illumina':int(rows[jid]['Illumina_fragment_ids']),'geometry':int(rows[jid]['Illumina_distinct_geometries']),'reads':rna,'control':sample['gene_name_5p']=='BCR' and sample['gene_name_3p']=='ABL1'})
 data={'candidates':candidates,'summary':load(OUT/'corroboration.json'),'cost':load(RUN/'resource-accounting.json'),'scope':'Five exact junctions supported in both direct-RNA libraries; not the full candidate pool','reference_sha256':plain.hexdigest()}
 ...
 template=(ROOT/'scripts/k562/dashboard.html').read_text();markup=template.replace('__DATA__',json.dumps(data,separators=(',',':')).replace('</','<\\/'))
 assert len(markup.encode())<1_000_000
 VIS.parent.mkdir(parents=True,exist_ok=True)
 VIS.write_text(markup);print(json.dumps({'visual':str(VIS),'bytes':len(markup.encode()),'candidates':len(candidates),'reads':sum(len(c['reads']) for c in candidates)}))
